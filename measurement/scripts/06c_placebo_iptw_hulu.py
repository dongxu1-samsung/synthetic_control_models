"""
Step 6c: Placebo test for IPTW (Hulu campaign 291260, flight 648768).

Refits a propensity model on the SAME covariates but against a random
pseudo-treatment label, then applies IPTW using those refit scores. If the
modeling+weighting pipeline is unbiased, the measured placebo lift should be
~0 regardless of which covariates went into the (here, uninformative-by-
construction) propensity model. This avoids the bug found in the Apple TV
pipeline's first placebo attempt, which incorrectly reused the REAL
propensity score against a random pseudo-split.
"""
import numpy as np
import pandas as pd
...
# Same collinearity/zero-variance drops as 02_fit_propensity_hulu.py
numeric_features = [c for c in numeric_features if c != "HIST_CTV_IMPRESSIONS_7D"]
log_transform_cols = [
    "APP_COUNT", "TOTAL_ACTIVE_DAYS", "TOTAL_MINUTES",
    "ACTIVE_APP_COUNT", "DORMANT_APP_COUNT", "SLEEPING_APP_COUNT", "AT_RISK_APP_COUNT",
    "CHAMPION_APP_COUNT", "REGULAR_APP_COUNT", "LIGHT_APP_COUNT",
...
print("\n--- Placebo test (IPTW, propensity refit on pseudo-split) ---")
