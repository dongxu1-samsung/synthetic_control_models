"""
Step 15: Propensity-based matching + IPTW on the MERGED Apple dataset
(campaign 296636 / flight 658095), using the PROPENSITY_SCORE fit in
14_fit_propensity_merged.py on the merged + deduped feature set.

Same methodology as the original 03_match_and_weight.py: common-support
trimming, 1:1 nearest-neighbor caliper matching (caliper=0.01), and
...
IN_PATH = "/Users/dongd1.xu/Documents/hermes_agent/measurement/data/merged_propensity_scored.parquet"
OUT_MATCHED_PATH = "/Users/dongd1.xu/Documents/hermes_agent/measurement/data/merged_matched_pairs.parquet"
OUT_WEIGHTED_PATH = "/Users/dongd1.xu/Documents/hermes_agent/measurement/data/merged_iptw_weighted.parquet"
...
# ==================================================================
# (a) 1:1 nearest-neighbor caliper matching
# ==================================================================
CALIPER = 0.01
...
print(f"\nMatching {len(treated)} treated to {len(control)} control candidates (caliper={CALIPER})...")
...
matched_pairs = pd.concat(
    [matched_treated.assign(GROUP="treated"), matched_control.assign(GROUP="control")],
    ignore_index=True,
)
...
# (b) Stabilized IPTW
# ==================================================================
p = df_trimmed["PROPENSITY_SCORE"].clip(1e-4, 1 - 1e-4)
treat = df_trimmed["EXPOSED"]
p_treat_marginal = treat.mean()
sw = np.where(treat == 1, p_treat_marginal / p, (1 - p_treat_marginal) / (1 - p))
