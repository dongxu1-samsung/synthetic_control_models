"""
Step 3b: Tie-robust matching alternative (Hulu campaign 291260, flight 648768).

The Apple TV pipeline's original 1:1 nearest-neighbor matcher failed its
placebo test due to tie-breaking bias in propensity score (heavy ties from
discrete covariates caused sklearn's NearestNeighbors to tie-break
deterministically by array position). Applying the same precautionary fix
here: add small random jitter before matching, repeat multiple times, and
average -- confirms whether the same issue exists for this population.
"""
import numpy as np
import pandas as pd
...
OUT_PATH = "/Users/dongd1.xu/Documents/hermes_agent/measurement/data/matched_pairs_derandomized_hulu.parquet"
