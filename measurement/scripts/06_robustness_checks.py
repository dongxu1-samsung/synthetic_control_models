...
(b) Investigate matched vs. IPTW divergence: compare effective sample size
    for matching (given heavy control reuse) and check whether restricting
...
# Randomly split control into two halves, pretend half is "pseudo-treated"
...
# the two pseudo-groups, since they were assigned randomly the propensity
# score itself is not predictive by construction -- this checks the
# machinery of the matching pipeline itself, not omitted-variable bias)
...
# Effective sample size accounting for reuse (Kish's approximation using
# reuse count as an implicit weight)
...
# Check: does the small unique control pool (228,698) systematically differ
# in propensity score from the full control pool (4,050,906)? If the matching
# caliper is pulling from a narrow high-propensity slice of control, that
# could explain the matched-vs-IPTW divergence (IPTW uses the full weighted
# control distribution, matching only uses whichever narrow slice sits near
# the treated group's propensity distribution).
